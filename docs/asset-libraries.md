...
| 3dtextures.me stylised textures (João Paulo "Gendo") | https://3dtextures.me (licence: https://3dtextures.me/about/) | CC0 ("All textures on this site are licensed as CC0"); checked 17 Sep 2026 | Hand-painted-look PBR sets (basecolor, normal, roughness, AO, height; mostly 1024 px) downloadable one by one from linked Google Drive folders. Dungeon fits: Stylized Stone Wall 001, Stylized Bricks 004 (mossy), Stylized Stone Floor 005/006, Stylized Wood Planks 002, Stylized Lava 001. Newer sets move to stylizedtextures.com. | **In use** by the first-person prototype (`www/assets/proto/fp/`, credits in `LICENSE.md` there): the painted look matches the monster art far better than photo textures. | None required (credited anyway). |
...
4. **3dtextures.me stylised stone/brick/floor/wood/lava sets** (CC0, now used in first-person); **ambientCG** / **Poly Haven** (CC0) remain the photoreal alternative.
...
- **First-person** (`www/assets/proto/fp/`): per surface a colour map `<stem>_color.jpg` and a normal map `<stem>_normal.jpg` (OpenGL convention), 512x512 seamless, for the stems `stone_wall`, `mossy_bricks`, `flagstone_floor`, `wood_planks` (256 px is enough), plus `lava_color.jpg`. `FpTextures` falls back to procedural stone per surface when a file is missing. To swap in another CC0 set: take the 1K basecolor + normal, resize to 512 px, save as JPG, and credit it in `www/assets/proto/fp/LICENSE.md`. Monster billboards stay in `www/assets/proto/monsters/<id>.png`.
