...
The monsters are pre-rendered from 3D models (image to 3D with TRELLIS, then `tools/monsters3d/render_monster_iso.py` renders walk, idle, attack and hit clips at the view's angle) in five facings that cover all eight screen directions, so Mr Owl and every monster face the way they are walking and turn as they go. In the encounter card the character is lifted off the painted scene onto its own layer, where it turns round to meet the player, squares up to answer and walks away beaten. Because the grounds are four times larger, the scene bakes ground, shadows and lantern pools into chunked textures and sorts props in depth bands; `?perf=1` shows frame rate, logic time, draw calls and the chunk pool. `npm run test:cem:perf` walks Mr Owl a long route in headless Chrome and prints the same readout: on a 60x52 cemetery it settles at 52 draw calls, 158 visible sprites, 46 top-level display objects with 609 props parked in bands, 17 live ground chunks and 0.02 ms of game logic per frame. Before the rewrite the same map put about 4400 objects on the display list and re-sorted all of them every frame.

Beating the Reaper plays a defeat choreography on the map before the result card (three hits, a purple blaze, collapse into a ring of light, his captured souls drifting off as wisps, the great tomb's glow turning gold); the dungeon's dragon now has one too (rears up roaring in a gold blaze, the lava cools, it bows its head and its hoard bursts into sparkles as it fades). The cemetery opens on a quiet nocturne after Solveig's Song (`cemetery-solveig`, two minutes) and fades over to the dark carnival waltz (`cemetery-quirky`) while the Grim Reaper is fought. Pick another with `?music=solveig|shanty|gothic|quirky|ominous|carousel|lullaby|tristram|none`; solveig and tristram are quieter wandering themes loosely after Grieg's Solveig's Song and the Tristram theme. Music plays at half the effects' level. All tracks come from ACE-Step (Apache-2.0) via `tools/music/generate_loop.py`, so they are clear for release.
