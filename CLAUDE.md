...
**Cemetery rendering** (cem-world.js): ground, prop shadows and lantern pools are baked into pooled 8x8-tile render textures (1024x696 each, pool capped at `CHUNK_POOL_MAX`); props live in depth-band Layers with cell culling, and `setActive` pauses the animation and the `cemTween` of whatever is culled or unlit, since Phaser advances both for invisible objects; the night is an erased fog texture that follows Mr Owl (`buildFog`). The model keeps `visChanged` (tiles whose `vis` value changed) next to `newlySeen`; the flow hands both to `onTilesRevealed`, so a step refreshes those tiles only and leaves the chunk bakes to the per-frame budget (a full pass with forced bakes happens once, at create). `?perf=1` shows frame rate, logic time, draw calls and the chunk pool. Boot: `IsoTextures.generateFallbacks` takes an `only` list (the cemetery needs a dozen of the dungeon's sixty textures), `CemTextures.generate` registers the stand-in props and paints one only when `sprite()` finds no kit image, the night sheet is painted only with `{ fog: true }`, and `SFX.warm()` builds the AudioContext behind the loading veil (built in-frame on the first footstep it froze the game 300 ms)
