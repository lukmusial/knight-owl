...
- Cemetery night-reveal check (headless Chrome; walks Mr Owl and fails if anything on screen pops from dark to lit within 3 frames, shows in the dark, or if a ground tile's baked-plus-sprite composite drifts from its peak): `npm run test:cem:reveal`
- Cemetery night-reveal clip (a 40 s slow walk toward a crypt, a lantern and a wanderer, encounters off): `npm run record:cem:reveal`
...
| CemReveal | `js/proto/cem-reveal.js` | Per-tile night reveal factor from Mr Owl's float position (pure, node-tested): live factor, monotonic peak, a point factor for things that move, a window around him only |
