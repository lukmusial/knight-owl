...
- Cemetery night-reveal clip (a 40 s walk toward a crypt, a wanderer and a second crypt, encounters off): `npm run record:cem:reveal`
...
| CemReveal | `js/proto/cem-reveal.js` | Per-tile night reveal factor from Mr Owl's float position (pure, node-tested): live factor, monotonic peak, quantised ground level, a window around him only |
