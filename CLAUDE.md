...
- Cemetery night-reveal check (headless Chrome; walks Mr Owl with a tap-to-walk leg and a pinch, and fails if anything on screen pops from dark to lit within 3 frames, shows in the dark, if a tile inside the view has no reveal after 3 frames, or if a ground tile's baked-plus-sprite composite drifts from its peak; `--zoom 0.8` / `--zoom 0.5` for the phone and pinched views): `npm run test:cem:reveal`
- Cemetery night-reveal clip (a 50 s slow walk toward a crypt, a pinch out and back, a lantern, a tap-to-walk pan and a wanderer, encounters off): `npm run record:cem:reveal`
...
| CemReveal | `js/proto/cem-reveal.js` | Per-tile night reveal (pure, node-tested): a distance factor from Mr Owl's float position over a window around him, a view fade for tiles inside the camera, a monotonic peak, a point factor for things that move |
