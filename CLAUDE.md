...
**Rain and puddles** (cem-rain.js decides, cem-scenes.js `buildRain`/`updateRain` draws): a gentle rain starts `RAIN_DELAY_MS` after the scene is ready and ramps in over `RAIN_RAMP_MS`; the streaks are one screen-space particle emitter (scrollFactor 0, one 4x30 texture, `RAIN_ALIVE` alive, slanted `RAIN_SLANT_DEG`, relaid on zoom and resize by `layoutRain`). Puddles (`cem_puddle_0..3`, procedural, a wet halo, water, a sky-coloured sheen and a gleam) lie on the floor layer (`addProp` with `ground: true`, so they are culled with their cell and sit under whoever stands in them) on lane and plaza tiles chosen by `hash(gx, gy, 41) < PUDDLE_CHANCE`. They are laid around Mr Owl (`PUDDLE_REACH` tiles) each time he changes tile, not on every seen tile: the lanterns reveal their surroundings from the start and would spend the cap (`MAX_PUDDLES`) far away. When the cap is reached the farthest puddle at least `RECYCLE_DIST` away and off screen is moved (`world.moveProp`). Each puddle fills over `PUDDLE_FILL_MS` after its own stagger (`puddleFill`), fog-tinted like every prop, and only the ones still changing are touched per frame (`settled`). A pooled ring (`RING_CAP`) lands on a puddle in view `RING_RATE` times a second; a step in a puddle (`splashStep`, throttled by `SPLASH_MS`) spawns two bigger rings, a few drops and the `splash` recipe in sfx.js instead of the dry footstep; Mr Owl's reflection is his own sheet flipped, `REFLECTION_ALPHA` times how full the puddle is. Under `REDUCED_MOTION` the puddles lie there full and still, with no streaks, rings or drops. The `?perf=1` overlay prints the rain's per-frame time, streaks, puddles and rings; `npm run test:cem:rain` checks the caps and the splash
