...
**Thunder and lightning** (cem-storm.js, `buildStorm`/`strikeLightning` in cem-scenes.js): every 25-60 s (`CemStorm.DEFAULTS`: first after 15 s, `retryMs` when a strike had to wait) lightning hits a lit tile (`vis === 2`) 2-8 tiles from Mr Owl - lantern light marks tiles lit all over the grounds, so `maxDist` is what keeps the strike on screen. The bolt is a seeded midpoint-displacement polyline with 2-3 branches from above the top of the view, stroked once per strike into two Graphics (an additive glow and a white core), with an additive pool and burst at its foot; it flickers for `boltMs` (150) through `boltAlpha`, the whole view lightens under a scrollFactor-0 `cem_flash` image (`flashAlpha`: `flashPeak` 0.7 decaying over `flashMs` 350 with a second pulse), props within `brightenRadius` tiles take a cold tint until the bolt is gone (`restoreProps` puts back the tint `refreshVisibility` would give), the camera shakes, and the `thunder` SFX recipe follows `thunderDelay` later (300-1200 ms, farther is later and quieter). The schedule is a scene TimerEvent, so pausing the game pauses the storm; a strike is skipped (and retried) while `ProtoCem.isBusy()`, input is off or the level is paused. Between strikes the only per-frame cost is one `stormStrike` check; `tickStorm` runs only during a strike. Reduced motion: no flash, shake or flicker, a dim bolt and the sound. The harness aims a strike with `strikeLightning({ target, seed })`
