...
- Cemetery rain check (headless Chrome, checks the rain clock runs as the scene leaves it, then winds it on, walks through the puddles, screenshots `docs/screenshots/cem-rain.png`, prints the rain's per-frame cost; `--reduced` runs it under prefers-reduced-motion): `npm run test:cem:rain`
- Cemetery on the phone (the debug app's WebView over adb and the DevTools protocol; prints the rain and storm state every second, `--goto` opens a page, `--eval` runs an expression): `adb forward tcp:9223 localabstract:webview_devtools_remote_<pid>` (find the socket in `adb shell cat /proc/net/unix | grep devtools`), then `node tools/smoke/cem-device-probe.js --goto "https://localhost/proto/isometric.html?name=Probe&action=new&level=cemetery&perf=1" --seconds 40`. The page must be visible on the screen (unlocked, app in front): Phaser does not create the scene while it is hidden
...
- Cemetery defeat check (headless Chrome; beats a wanderer of each exit style and the Reaper, records the sprite's alpha every frame and fails if it ever rises again before the sprite is destroyed): `npm run test:cem:defeat`
...
**Lantern light**: the kit lamp post (`lantern_post.png`) hangs its lamp from an arm on the post's screen-left side, not on its tip. The manifest's `light` point (from `light: [0, -0.33, 1.19]` in `tools/iso/cemetery_models.json`, tile units from the footprint centre) is the centre of the lamp's glass; `buildLanterns` puts the glow there and a `FLAME_SCALE` flame standing `FLAME_LIFT` px under it, inside the glass. `CemModel.CONFIG.LAMP_OFFSET` (+0.33 gy) is the same offset on the floor: `CemModel.lampPoint` gives the point the light map, the cast shadows, the baked floor pool and the fog glow use, while `level.lights[].gx/gy` stay the post's tile for the reveal, visibility and minimap. `tools/smoke/cem-lantern-shot.js` takes a close-up to check it, `tools/smoke/png-probe.js` measures a sprite
