...
async function main() {
  ...
  try {
    ...
    const after = await page.evaluate(() => {
      ...
      // the props got the reveal's tint back: compare with what refreshVisibility (every tile through applyTile) gives
      const before = S.tileProps.flat().map(p => p.tintTopLeft);
